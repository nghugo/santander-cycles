...
const StationCard = ({ station, prefix }) => {
  ...
  return (
    <Card elevation={3} sx={{ p: 1, height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between"}}>
      <div>
        <Typography variant="subtitle2" sx={{ fontSize: 16 }}>
          {prefix}
        </Typography>
      ...
      <p>{station.name1}</p>
      <p>{station.name2}</p>
      <p>Cycles Available: {station.nbBikes}</p>
      <p>Standard:{station.nbStandardBikes}, Electric:{station.nbEBikes} </p>
      <p>Empty Docks:{station.nbEmptyDocks} </p>
      <p>Total Docks:{station.nbDocks} </p> */}
    </Card>
  );
};
